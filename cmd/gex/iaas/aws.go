...
func (this *aws) Execute(shoot gube.Shoot, config map[string]string, args ...string) error {
	...
	err := util.ExecCmd("aws "+strings.Join(args, " "), "AWS_ACCESS_KEY_ID="+string(config["accessKeyID"]), "AWS_SECRET_ACCESS_KEY="+string(config["secretAccessKey"]), "AWS_DEFAULT_REGION="+region, "AWS_DEFAULT_OUTPUT=json")
	if err != nil {
